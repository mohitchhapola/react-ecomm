  ...
  const dispatch = useDispatch()
    ...
    const handleCart = (e) => {
      ...
      // if (items.findIndex((item) => item.product.id === product.id) <  0) {
        // console.log( product );
        const newItem = {
          product:product.id,quantity:1,user:user.id,color1:selectedColor.name,size1:selectedSize.name
        };
        delete newItem['id']
        // if (selectedColor) {
      ...
      <div className="mx-auto max-w-2xl px-4 pb-16 pt-10 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pb-24 lg:pt-16">
        ...
        <div className="mt-4 lg:row-span-3 lg:mt-0">
          <div className="flex items-center justify-between">
                ...
                <button  
                // onClick={handlelike()}
                className="text-3xl font-medium text-red border-black hover:text-red-600">
                  &#9829;
                </button>
              </div>
                        ...
                        {product.colors.map((color) => (
                          <RadioGroup.Option
                            ...
                            className={({ active, checked }) =>
                              classNames(
                                color.selected ? 'ring ring-offset-1' : '', // Apply ring class if selected
