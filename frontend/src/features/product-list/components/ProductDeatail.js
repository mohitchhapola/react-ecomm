...
// console.log(user.user.id)
    const newItem = {
      ...
      quantity: 1,
      user: user.user.id,
      color1: selectedColor?.name,
      size1: selectedSize?.name,
    };
                ...
                <RadioGroup value={selectedColor} onChange={setSelectedColor} className="mt-4">
                  <div className="flex items-center space-x-3">
                    {product.colors.map((color) => (
                      <RadioGroup.Option key={color.name} value={color} className={({ active, checked }) => classNames('relative flex items-center justify-center rounded-full p-0.5')}>
                        <RadioGroup.Label as="span" className="sr-only">
                          {color.name}
                        </RadioGroup.Label>
                        <span className={classNames(color.class, 'h-8 w-8 rounded-full border border-black border-opacity-10')} />
                      </RadioGroup.Option>
                    ))}
                  </div>
                </RadioGroup>
